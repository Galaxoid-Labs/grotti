...
// Throughput benchmark for the Metal compute hasher — measures sustained hashrate on the
// Apple GPU so METAL_EST_HPS (the cap-split estimate in cli/main.odin) reflects reality.
// Mirrors vulkan/bench.
//
//   odin run metal/bench -o:speed
...
main :: proc() {
	...
	// Sweep launch sizes to separate compute throughput from per-dispatch overhead. On an
	// M1 Max this is flat (~0.22 GH/s) from 2^22 up — the kernel is ALU-bound on integer/
	// rotate throughput (Apple GPUs have no hardware funnel-shift), not dispatch-bound, so
	// the smallest responsive launch is chosen in metal_worker.odin at no throughput cost.
	for shift in u32(22) ..= u32(27) {
		...
		metalbackend.engine_scan(&e, 0, launch, hits[:]) // warmup

		start := time.now()
		total: u64
		for i in 0 ..< iters {
			metalbackend.engine_scan(&e, u32(i) * launch, launch, hits[:])
			total += u64(launch)
		}
