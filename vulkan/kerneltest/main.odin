...
// Correctness gate for the Vulkan compute hasher — the green light that de-risks the
// whole backend (CLAUDE.md invariant #4). Same two checks as cuda/kerneltest:
//   1. Reproduce block 125552's winning nonce (real-data anchor).
//   2. Differential vs the CPU scan_simd over the same range on an easy target — Vulkan
//      must find EXACTLY the same hits. A fast-but-wrong kernel is worse than none.
//
//   odin run vulkan/kerneltest
...
main :: proc() {
	...
	// 1. Block 125552 anchor.
	{
		...
		hits: [16]u32
		n := vkbackend.engine_scan(&e, WINNER - 100, 4096, hits[:])
	...
	// 2. Differential vs CPU over the same range, easy target.
	{
		...
		vkbackend.engine_load_job(&e, mid, w0, w1, w2, easy)

		gpu: [2048]u32
		cpu: [2048]u32
		gn := vkbackend.engine_scan(&e, 0, 65536, gpu[:])
		...
		fmt.printfln("differential: Vulkan %d hits vs CPU %d hits — %s", gn, cn, match ? "IDENTICAL" : "MISMATCH")
		pass &= match
	}

	fmt.println(pass ? "PASS: shader is correct — safe to optimize / integrate." : "FAIL: do NOT proceed.")
}
